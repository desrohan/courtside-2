...
import { governanceConfigs, governingBodies, leagues as allLeagues, type LeagueCompetition } from '@/data/governance';

export default function LeaguesTab() {
  const config = governanceConfigs[0];
  const items = config.leagues.length > 0 ? config.leagues : allLeagues.filter(l => l.sport === config.sport);
  const [showAdd, setShowAdd] = useState(false);

  const typeColors: Record<string, string> = { league: 'bg-green-50 text-green-600', tournament: 'bg-orange-50 text-orange-600', cup: 'bg-blue-50 text-blue-600' };
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="bg-white rounded-2xl border border-dark-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-dark-100 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-dark-900">Leagues & Competitions</h2>
            <p className="text-[11px] text-dark-400 mt-0.5">Tournaments, leagues, and cups linked to governing bodies</p>
          </div>
          ...
          <div className="px-5 py-4 border-b border-dark-100 bg-dark-50/30">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className="mt-1 w-full h-9 px-3 rounded-xl border border-dark-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-court-500/20" placeholder="Competition name" />
              </div>
              <div>
              ...
              <div>
                <label className="text-[11px] font-semibold text-dark-400 uppercase tracking-wider">Governing Body</label>
                <select className="mt-1 w-full h-9 px-3 rounded-xl border border-dark-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-court-500/20">
              ...
              <div>
                <label className="text-[11px] font-semibold text-dark-400 uppercase tracking-wider">Season</label>
                <input className="mt-1 w-full h-9 px-3 rounded-xl border border-dark-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-court-500/20" placeholder="e.g. 2025-26" />
              </div>
            </div>
        ...
        {items.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <Trophy size={32} className="mx-auto text-dark-300 mb-2" />
            <p className="text-sm text-dark-400">No leagues or competitions configured</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="bg-dark-50/60">
                ...
                <th className="text-left px-5 py-2.5 text-[11px] font-bold uppercase tracking-wider text-dark-400">Season</th>
                <th className="text-left px-5 py-2.5 text-[11px] font-bold uppercase tracking-wider text-dark-400">Body</th>
                <th className="text-right px-5 py-2.5 text-[11px] font-bold uppercase tracking-wider text-dark-400">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-dark-100">
              {items.map((league: LeagueCompetition) => {
                const body = governingBodies.find(b => b.id === league.governingBodyId);
                return (
                  <tr key={league.id} className="hover:bg-dark-50/30 transition-colors">
                    <td className="px-5 py-3 text-sm font-semibold text-dark-800">{league.name}</td>
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold capitalize ${typeColors[league.type] || ''}`}>{league.type}</span>
                    </td>
                    <td className="px-5 py-3 text-xs text-dark-500">{league.level}</td>
                    <td className="px-5 py-3 text-xs text-dark-500">{league.season || '—'}</td>
