...
interface CycleModalProps {
  cycle: PlannerCycle | null;
  seasonId: string;
  onClose: () => void;
  onSave: (c: PlannerCycle) => void;
}

export default function CycleModal({ cycle, seasonId, onClose, onSave }: CycleModalProps) {
  const [name, setName] = useState(cycle?.name || '');
  const [startDate, setStartDate] = useState(cycle?.startDate || '');
  const [endDate, setEndDate] = useState(cycle?.endDate || '');
  const [color, setColor] = useState(cycle?.color || '#C8E6C9');

  const handleSave = () => {
    if (!name || !startDate || !endDate) return;
    onSave({
      id: cycle?.id || `cyc-${Date.now()}`,
      seasonId,
      name,
      startDate,
      endDate,
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-dark-100">
          <h3 className="text-base font-bold text-dark-900">{cycle ? 'Edit Cycle' : 'Create Cycle'}</h3>
